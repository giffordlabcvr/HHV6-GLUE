...
glue.inMode("module/hhv6FastaAlignmentExporterSeqIdOnly", function() {
	nucleotideAlmt = glue.command(["export", "AL_HHV6B", 
			"-r", "REF_HHV6B", "-f", "whole_genome", 
			"-a", 
			"-p"]);
});
...
_.each(nucleotideAlmt.nucleotideFasta.sequences, function(membObj) {
	...
	glue.inMode("sequence/ncbi-curated-hhv6a/"+membObj.id, function() {
